...
class UserConnection(DiscordModelsBase):
    ...
    async def fetch_from_api(cls, cache=True):
        ...
        API call to Discord. If an instance of :py:class:`quartcord.User` exists in the users internal cache
        who are attached to these connections then, the cached property :py:attr:`quartcord.User.connections`
        is updated.

        Parameters
        ----------
        cache : bool
            Determines if the :py:attr:`quartcord.User.guilds` cache should be updated with the new guilds.

        Returns
        -------
        list[quartcord.UserConnection, ...]
            List of instances of :py:class:`quartcord.UserConnection` to which this user belongs.
